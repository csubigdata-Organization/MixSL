...
def create_subgraphs3(data, h=2, sample_ratio=1.0, max_nodes_per_hop=None,
...
def create_subgraphs2(data, h=2, sample_ratio=1.0, max_nodes_per_hop=None,
    ...
    for h_ in h:
        ...
        for ind in range(num_nodes):
            ...
            # subgraph_node_to_subgraph = torch.zeros([data_.num_nodes])
            if center_idx:
                data_ = subgraph_to_subgraph2_with_idx(data_, h_, node_label=node_label, use_rd=use_rd,
                                                       self_loop=self_loop)
            else:
...
def create_subgraphs(data, h=2, sample_ratio=1.0, max_nodes_per_hop=None,
...
def subgraph_to_subgraph4(d, num_hops, center=0, node_label='spd', use_rd=False):
    ...
    for i, n in enumerate(neighbors):
        ...
        if node_label.startswith('spd'):
            ...
            z[i * d.num_nodes: (i + 1) * d.num_nodes] = torch.cat([d.z, z_n + num_hops + 3],
                                                                  dim=-1)  # +num_hops+3 such that it uses different distanc embedding
        elif node_label.startswith('dspd'):
            z_n = find_all_spd(n, edge_index=d.edge_index, node_label=node_label, num_nodes=d.num_nodes)
