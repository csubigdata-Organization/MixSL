...
from Backbone_Models.utils import KMISPooling


class k_MISPool(torch.nn.Module):
    

    def __init__(self, args, gnn_layers=2):

        super(k_MISPool, self).__init__()
        ...
        self.hierarchical_poolings = nn.ModuleList()

        for i in range(self.gnn_layers):
            nn1 = Sequential(Linear(self.hidden_dim, self.hidden_dim), ReLU(), Linear(self.hidden_dim, self.hidden_dim))
            conv = GINConv(nn=nn1, aggr='add')
            self.convs.append(conv)
            pooling = global_add_pool
            self.global_poolings.append(pooling)
            hierarchical = KMISPooling(self.hidden_dim, k=5, aggr_x='sum')
            self.hierarchical_poolings.append(hierarchical)
    ...
    def forward(self, data):
        ...
        for i in range(self.gnn_layers):
            ...
            x, edge_index, _, batch = self.hierarchical_poolings[i](x, edge_index, batch=batch)[:4]
